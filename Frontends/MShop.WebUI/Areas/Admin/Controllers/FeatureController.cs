...
namespace MShop.WebUI.Areas.Admin.Controllers
{
    ...
    public class FeatureController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7205/api/Features/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        [HttpGet]
        public async Task<IActionResult> UpdateFeature([FromRoute] string id)
        {
            var client = _httpClientFactory.CreateClient();
        ...
        public async Task<IActionResult> UpdateFeature(UpdateFeatureDto dto)
        {
            ...
            var stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7205/api/Features", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
